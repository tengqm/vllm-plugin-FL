...
"""GCU fix for chunk_gated_delta_rule_fwd_h Triton kernel.

GCU hardware limits grid.y and grid.z to 255.  The upstream launcher uses
``grid = (cdiv(V, BV), N * H)`` which fails when ``N * H > 255`` (e.g.
varlen prefill with 33+ sequences and H=8 → grid.y=264).
"""
...
def chunk_gated_delta_rule_fwd_kernel_h_blockdim64_gcu(
    k,
    v,
    w,
    v_new,
    ...
    n_v_blocks = tl.cdiv(V, BV)
    i_nh = flat_pid // n_v_blocks
    if i_nh >= NH_TOTAL:
        return
...
def _gcu_grid(V: int, nh_total: int, meta: dict) -> tuple[int, int]:
    n_v = triton.cdiv(V, meta["BV"])
    total = n_v * nh_total
...
def chunk_gated_delta_rule_fwd_h_gcu(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    ...
    def grid(meta):
        return _gcu_grid(V, nh_total, meta)

    chunk_gated_delta_rule_fwd_kernel_h_blockdim64_gcu[grid](
        k=k,
        v=u,
        w=w,
        v_new=v_new,
...
def apply_chunk_delta_h_gcu_patch() -> None:
    """Patch FLA chunk_delta_h ops for GCU grid limits."""
    global _patched
    if _patched:
        return
    ...
    try:
        import vllm.model_executor.layers.fla.ops.chunk as chunk_mod
        import vllm.model_executor.layers.fla.ops.chunk_delta_h as chunk_delta_h
        import vllm.model_executor.layers.fla.ops.kda as kda_mod
        ...
        logger.warning("Failed to patch chunk_gated_delta_rule_fwd_h for GCU: %s", exc)
